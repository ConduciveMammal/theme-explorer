...
const ThemeAccordion = ({theme, shop, index}) => {
  ...
  return (
      <div className="Accordion__Container">
        <header className={`Accordion__Header${theme.role === 'main' ? ' Accordion__Header--main' : ''}${isOpen ? ' Accordion__Header--open' : ''}`} onClick={onToggle} aria-controls={`Accordion-${index}`}>
          <div className="Accordion__Icon-container">
            <Icon name="theme" color="#FFFFFF" size={35} classes={'Accordion__Icon'}/>
          </div>
          <p className="Accordion__Title">{theme.name}</p>
          <p className="Accordion__Link">
            <a href={theme.role === 'main' ? `https://${shop.domain}` : themePreviewUrl} target="_blank" rel="noreferrer">{theme.role === 'main' ? 'View' : 'Preview'}</a>
          </p>
        </header>
        <Collapse
          id={`Accordion-${index}`}
          ...
          <div className="Accordion__Body__Content">
            ...
            <p className="Accordion__Detail">
              ...
              <span className="Accordion__Value"><DisplayDate date={theme.created_at} /></span>
            </p>
            <p className="Accordion__Detail">
            ...
            <footer className="Accordion__Footer">
              <p className="Footer__Link">
                <a href={themeJsonUrl} target="_blank" rel="noreferrer">View JSON</a>
              </p>
              <p className="Footer__Link">
                <a href={themeCustomiseUrl} target="_blank" rel="noreferrer">Customise</a>
              </p>
              <p className="Footer__Link">
                <a href={themeCodeUrl} target="_blank" rel="noreferrer">Edit code</a>
              </p>
            </footer>
          </div>
        </Collapse>
